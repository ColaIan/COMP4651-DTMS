...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Instructor Availability</h1>
    ...
    <form method="POST" action="?/create" class="mb-6">
        ...
        <button type="submit" class="rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-600">Add Availability</button>
    </form>
        ...
        <table class="w-full table-auto border-collapse border border-gray-300">
            ...
            <tbody>
                {#each props.data.availabilities as availability}
                    <tr>
                        <td class="border border-gray-300 px-4 py-2">{new Date(availability.startTime).toLocaleString()}</td>
                        <td class="border border-gray-300 px-4 py-2">{new Date(availability.endTime).toLocaleString()}</td>
                        <td class="border border-gray-300 px-4 py-2"><form method="POST" action="?/delete"><input type="hidden" name="id" value={availability.id}><button class="text-red-800 cursor-pointer">Delete</button></form></td>
                    </tr>
                {/each}
            </tbody>
        </table>
